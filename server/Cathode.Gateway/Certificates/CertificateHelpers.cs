// From https://github.com/dotnet/wcf/blob/main/src/System.Private.ServiceModel/src/System/IdentityModel/Claims/X509CertificateClaimSet.cs
...
namespace Cathode.Gateway.Certificates
{
    ...
    {
        public static IList<string> GetAllNames(this X509Certificate2 certificate)
        {
            var results = new List<string>();
            results.Add(certificate.GetCommonName());
            results.AddRange(GetDnsFromExtensions(certificate));
            return results;
        }
            ...
            foreach (X509Extension ext in cert.Extensions)
            {
                ...
                if (ext.Oid.Value == X509SubjectAlternativeNameConstants.Oid)
                {
                    ...
                    string[] rawDnsEntries =
                        asnString.Split(new string[1] { X509SubjectAlternativeNameConstants.Separator },
